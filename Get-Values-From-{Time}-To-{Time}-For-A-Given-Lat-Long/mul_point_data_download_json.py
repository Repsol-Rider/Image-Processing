...
output_folder = r'JSON/' # if r'' the location of the script is where the files will be outputted.
...
for Long,Lat,Serial in zip(pointsDataFrame['X'],pointsDataFrame['Y'],pointsDataFrame['sno']):
    ...
    output_file_location = os.path.join(output_folder, os.path.basename(str(Serial)+".json"))
